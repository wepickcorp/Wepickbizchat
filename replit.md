...
- **BizChat MMS/RCS API 규격**:
  ...
  - **광고 수신동의 멘트 (폼 디폴트로 이동)**: 더 이상 발송 시점에 안내 멘트(`※ 이 메시지는 SK텔레콤에서…`), 마무리 인사(`감사합니다.`), LMS 무료 수신거부(`무료 수신거부 1504`), `(광고)` 본문 접두사를 자동으로 합성하지 않습니다. 대신 "메시지 만들기"(`client/src/pages/templates-new.tsx`) 폼의 본문(`content`/`lmsContent`) 디폴트 값으로 안내 멘트가 미리 채워져 사용자가 화면에서 보는 본문 = 실제 발송되는 본문이 일치합니다. 메시지 유형을 LMS/MMS↔RCS로 토글하면 본문이 디폴트 그대로(또는 비어 있을 때)에 한해 새 유형의 디폴트로 교체되며, 한 번이라도 사용자가 직접 수정한 본문은 건드리지 않습니다. 기존 템플릿 수정/추천 템플릿 불러오기 흐름에서는 디폴트가 적용되지 않습니다. 발송 시 `mms.msg`와 `rcs[].msg`는 사용자가 저장한 본문이 그대로 들어갑니다. (단, `mms.title`의 30자 자르기와 `(광고)` 접두사 누락 시 자동 추가 안전장치는 E000002 방지를 위해 그대로 유지됩니다.)
