...
회사는 이용자가 서비스 이용 중 필요하다고 인정되는 각종 정보 및 광고를 전자우편, 휴대폰 메시지, 전화, 우편 등의 방법으로 제공할 수 있습니다. 이용자는 이를 원하지 않을 경우 수신을 거부할 수 있습니다.
...
export default function AuthPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const { error } = isLocalDev
      ? await localDevLogin(loginEmail, loginPassword)
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const { error } = isLocalDev
      ? await localDevSignup(signupEmail, signupPassword, signupName)
  ...
  const localDevLogin = async (email: string, password: string) => {
    try {
      ...
      localStorage.setItem("localDevAuth", "1");
      return { error: null };
    } catch (error) {
  ...
  const localDevSignup = async (email: string, password: string, name: string) => {
    try {
  ...
  const handleLocalDevStart = async () => {
    setIsLoading(true);
    ...
    <div className="min-h-screen bg-muted/30 px-4 py-8">
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-md items-center justify-center">
        <Card className="w-full rounded-2xl border bg-background shadow-sm">
          ...
          <CardContent>
          {isLocalDev && (
            <div className="mb-4 rounded-lg border border-primary/20 bg-accent p-4">
              <p className="text-sm font-semibold">로컬 개발 모드</p>
              <p className="mt-1 text-small text-muted-foreground">
                Docker 로컬 DB의 테스트 계정으로 바로 확인할 수 있어요.
              </p>
              <Button
                type="button"
                className="mt-3 min-h-11 w-full"
                onClick={handleLocalDevStart}
                disabled={isLoading}
                data-testid="button-local-dev-login"
              >
                16,000C 테스트 계정으로 시작하기
              </Button>
            </div>
          )}
          <Tabs defaultValue="login" className="w-full">
            ...
            <TabsContent value="signup">
              <form onSubmit={handleSignup} className="space-y-4 mt-4">
                <div className="rounded-lg border bg-muted/40 p-3 text-small text-muted-foreground">
                  가입만으로 크레딧은 지급되지 않아요. 가입 후 크레딧 화면에서 필요한 상품을 선택해 충전해요.
                </div>
                <div className="space-y-2">
